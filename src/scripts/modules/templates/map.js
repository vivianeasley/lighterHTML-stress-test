...
export const rows = function rows (state) {
    ...
    function getTileStyle (data) {
        if (!data) return
        const rgbArr = colors[data.biome];
        const newRbgArr = [];
        for (let index = 0; index < rgbArr.length; index++) {
            const value = rgbArr[index] + (data.elevation*3);
            const maxed = value > 255 ? 255 : value;
            newRbgArr.push(maxed)

        }
        return newRbgArr.join(", ")
    }
    return html`
        ${
                ...
                <div class="row" data-i=${i}>
                    ${
                        rowData.map((tileData, j) => html`
                            <div class="tile" style="${"background-color:rgb("+getTileStyle(tileData)+")"}" data-i=${j}>
                                ${meepleSVG(character, j, i)}
                            </div>
                        `)
                    }
                </div>
